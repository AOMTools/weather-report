...
v 1.02 Added functionality of relock and better logging method
...
class GuiPart:
    def __init__(self, master, queue, endCommand):
        ...
        self.set_value = [23.3, 10, 100, 10]   # Set value for parameters
...
class ThreadedClient:
    ...
    def initialiseParameters(self):
        ...
        self.params_value = [0, 50, 0, 5]   # Ugly fix
    ...
    def periodicCall(self):
        ...
        if self.gui.set_lock.get() == 1 and self.status != 2:    # If lock is set (but not in dangerous condition)
            self.status = 1
        elif self.gui.set_lock.get() == 0:
            self.status = 0
    ...
    def process_lock(self):
        ...
        if self.counter_ref > num_of_avg:
            ...
            # Write to the file
            with open("log/control_"+now1, "a") as myfile:
                myfile.write(now2 + ' ' + entry0 + ' ' + entry1 + ' ' + entry2 + ' ' + entry3 + ' ' + param0 + ' ' + param1 + ' ' + param2 + ' ' + param3 + ' ' + out + '\n' )
            ...
            if outvolt < 2 or outvolt > 9:
                ...
                sp.call(['./telegram_report.sh',USER,message])
                message = 'WAITING for the problem to hopefully solve itself ###'
                sp.call(['./telegram_report.sh',USER,message])
                print "The cavity drifts uncontrollably. Wait till coast is clear."
                self.status = 2  # dangerous condition
    ...
    def coast_clear(self):

        # Check whether coast is clear
        delta = self.temperature_value[0] - self.entry[0]
        if abs(delta) < 0.01:   # Difference less than 10 mK
            message = '### Message from Shiva and Ifrit. Looks that the coast is clear and the problem solves itself!!!'
            sp.call(['./telegram_report.sh',USER,message])
            message = 'Temperatures around the chamber is ' + self.gui.temperature_disp[0]['text'] + ' ' + self.gui.temperature_disp[1]['text'] + ' ' + self.gui.temperature_disp[2]['text'] + ' ' + self.gui.temperature_disp[3]['text']
            sp.call(['./telegram_report.sh',USER,message])
            message = 'That is all. Hopefully this does not happen very often ###'
            sp.call(['./telegram_report.sh',USER,message])
            print "The coast is clear. Starting the lock again"
            self.status = 1  # Reset the lock to work 

        # Resetting and displaying the reset values
        self.params_value = [0, 50, 0, 5]
        self.gui.params_value = self.params_value
        for i in range(4):
            self.gui.params_disp[i]['text'] = '%.3f'%round(self.gui.params_value[i],3)
    ...
    def workerThread1_UAIO(self):
        ...
        while self.running:
            try:
                ...
                # Write to the file (once every 10 seconds)
                if int(now2)%10 == 0:
                    with open("log/temp_"+now1, "a") as myfile:
                        myfile.write(now2 + ' ' + temp0 + ' ' + temp1 + ' ' + temp2 + ' ' + temp3 + '\n' )
...
root = Tkinter.Tk(  )
root.title("Weather Report v1.02 With Telegram")
